...
def _build(description, language, output_path, args, timeout, speak=None, player=None) -> str:
    ...
    if player:
        player.append_log("FRIDAY", "[Code] Build started...")
    ...
    for attempt in range(1, MAX_BUILD_ATTEMPTS + 1):
        ...
        if player:
            player.append_log("FRIDAY", f"[Code] Attempt {attempt}...")
        ...
        if player:
            player.append_log("FRIDAY", f"[Code] Fixing (attempt {attempt})...")
...
def _write_action(description, language, output_path, player) -> str:
    ...
    if player:
        player.append_log("FRIDAY", "[Code] Writing code...")
    try:
...
def _edit_action(file_path, instruction, player) -> str:
    ...
    if player:
        player.append_log("FRIDAY", "[Code] Editing file...")
...
def _explain_action(file_path, code, player) -> str:
    ...
    if player:
        player.append_log("FRIDAY", "[Code] Analyzing code...")
...
def _run_action(file_path, args, timeout, player) -> str:
    ...
    if not p.exists():
        return f"File not found: {file_path}"
    if player:
        player.append_log("FRIDAY", f"[Code] Running {p.name}...")
...
def _optimize_action(file_path, code, language, output_path, player) -> str:
    ...
    if player:
        player.append_log("FRIDAY", "[Code] Optimizing code...")
...
def _screen_debug_action(description, file_path, player, speak=None) -> str:
    ...
    if player:
        player.append_log("FRIDAY", "[Code] Taking screenshot for analysis...")
