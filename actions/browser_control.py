...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".friday_profiles" / browser
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".friday_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using FRIDAY profile")
                jarvis = str(Path.home() / ".friday_profiles" / "firefox_jarvis")
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".friday_profiles" / "safari")
        ...
        jarvis_profile = str(Path.home() / ".friday_profiles" / self.browser_name)
        Path(jarvis_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with FRIDAY profile: {jarvis_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(jarvis_profile, **kwargs)
            await asyncio.sleep(0.5)
            self._page = await self._context.new_page()
            print(f"[Browser] ✅ Launched [{label}] with FRIDAY profile")
...
def browser_control(parameters: dict = None, player=None, **_) -> str:
...
def _log(player, text: str):
    ...
    if player:
        player.append_log("FRIDAY", "[browser] " + short[:60])